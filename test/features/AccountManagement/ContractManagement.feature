...
Feature: Contract Management features
 ...
 Scenario Outline: Invalid required fields
  ...
  # Commenting the TC as this is no longer valid, as contract rate values will be rounded up
 ...
 Scenario Outline: Contract that is not eligible for ODTI Service - All the available Contract Rates have Minimum Rate defined and verified by adding Assignment type by selecting ‘Use Minimum Rate 2’ option
  When I login with "<username>" and "<password>"
  And I click account management link
  And I search for contract title "<contract title>"
  And I click the contract link "<contract title>" from search results
