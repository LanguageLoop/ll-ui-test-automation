...
Given(/^the looped in login page is opened$/,  function(){
 ...
 When(/^I login with "(.*)" and "(.*)"$/,  function(username,password){
  /*  if(username.includes("LLAdmin"))
    ...
    browser.waitUntil(()=>browser.getTitle()==="HomePage" || browser.getTitle()==="Bookings" || browser.getTitle()==="Account Management" || browser.getTitle().trim()==="Interpreters" ,{timeout:20000, timeoutMsg:'login not happened within 20s', interval:500 })
